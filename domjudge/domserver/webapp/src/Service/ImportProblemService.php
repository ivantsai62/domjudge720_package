...
class ImportProblemService
{
    ...
    public function importZippedProblem(
        ...
        //新增 restriction_languages 等需要 import 到 problem 等內容，基本從 zip 裡頭的 domjudge-problem-ini
        ...
        foreach (['sample', 'secret'] as $type) {
            ...
            foreach ($dataFiles as $dataFile) {
                ...
                //將 .task 數值存入 database
                if (($taskFile = $zip->getFromName(sprintf('data/%s/%s.task', $type, $dataFile))) !== false) {
                    $testcase->setTask((int)$taskFile);
                }
                if ($imageFile !== false) {
                ...
                $messages[] = sprintf('Added %s testcase from: <tt>%s.{in,ans,task}</tt>', $type, $dataFile);
            }
