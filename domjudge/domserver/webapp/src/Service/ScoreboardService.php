...
class ScoreboardService
{
    ...
    public function calculateScoreRow(
        ...
        //dump(json_encode($all_task_point));
        ...
        $this->em->getConnection()->executeQuery('REPLACE INTO scorecache
            (cid, teamid, probid,
             ...
             submissions_public, pending_public, solvetime_public, is_correct_public, is_first_to_solve, all_task_point)
            VALUES (:cid, :teamid, :probid, :submissionsRestricted, :pendingRestricted, :solvetimeRestricted, :isCorrectRestricted,
            :submissionsPublic, :pendingPublic, :solvetimePublic, :isCorrectPublic, :isFirstToSolve, :alltaskpoint)', $params);
        }
        else
        {
            $this->em->getConnection()->executeQuery('REPLACE INTO scorecache
        ...
        if ($updateRankCache && (($correctJury || $correctPubl) || !empty($task_point_result[1]) )) {
    ...
    public function updateRankCache(Contest $contest, Team $team)
    {
        ...
        foreach ($scoreCacheRows as $scoreCache) {
            foreach ($variants as $variant => $isRestricted) {
                ...
                $subtask = $scoreCache->getProblem()->getSubtask();
                ...
                else if (isset($contestProblems[$probId]) && !empty($subtask) )
                {
                    $all_task_point = $scoreCache->getAllTaskPoint();
                    if(!empty($all_task_point[0]) && !empty($all_task_point[1]))
                    {
                    $total_point = $contestProblems[$probId]->getPoints() * $all_task_point[1] / $all_task_point[0];
                    $numPoints[$variant] += $total_point;
                    }
    ...
    protected function getProblems(Contest $contest)
    {
        $queryBuilder = $this->em->createQueryBuilder()
            ->from(ContestProblem::class, 'cp')
            ->select('cp, partial p.{probid,externalid,name,subtask}')
